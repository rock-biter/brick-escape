...
export default class Player {
	...
	addLight() {
		this.light = new THREE.PointLight('#dba98c', 0, 30, 3)
	...
	timer = () => {
		...
		this.timeIndicator.innerHTML = `${minutes.toFixed(0)}:${
		...
		gsap.to(this.mesh.position, {
			...
			onComplete: () => {
				if (cell.isExit) {
					...
					gsap.to(cell.walls[1].material.color, {
						duration: 3,
						r: 0.29,
						g: 0.78,
						b: 0.02,
					...
					const nextCell = cell.connections.find(
						(el) => el.index !== this.currentCell.index
					)
	...
	moveUp() {
		...
		const n = this.currentCell.connections.find(
			(el) => el.index == neighborIndex
		)
